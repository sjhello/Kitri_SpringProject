				...
				<div class="tabs tabs-bordered clearfix" id="tab-1">
					...
					<ul class="tab-nav clearfix" id="head1">
						...
						<c:forEach var="list" items="${list}" varStatus="status">
							<li><a href="#tabs-${status.count+1}">${list.mealtime }</a></li>
						</c:forEach>
					</ul>
					...
					<div class="tab-container" id="body1">
						...
						<c:forEach var="list" items="${list}" varStatus="status">
							<div class="tab-content clearfix" id="tabs-${status.count+1}">
								<c:if test="${list.f1 != null }"><h2>One</h2></c:if>
								${list.f1 }
								<c:if test="${list.f2 != null }"><h2>Two</h2></c:if>
								${list.f2 }
								<c:if test="${list.f3 != null }"><h2>Three</h2></c:if>
								${list.f3 }
								<c:if test="${list.f4 != null }"><h2>Four</h2></c:if>
								${list.f4 }
								<c:if test="${list.f5 != null }"><h2>Five</h2></c:if>
								${list.f5 }
								<c:if test="${list.f6 != null }"><h2>Six</h2></c:if>
								${list.f6 }
							</div>
						</c:forEach>
					</div>
